...
export const createUser = ({ email, password, name }) => app.service('/api/users').create({ email, password, name })
export const generateToken = id => app.service('/api/token').get(id)
export const getUser = (email, password) => app.service('/api/users').find({ query: { email, password } })
